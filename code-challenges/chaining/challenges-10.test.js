...
const count = (target, input) =>
  input.reduce(
    (a, b) => (a += b.reduce((a, b) => (b === target ? a + 1 : a), 0)),
    0
  );
...
const totalSum = (input) =>
  input.reduce((a, b) => a + b.reduce((c, d) => c + d, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) =>
  input.map((a) =>
    a.filter((b) => !(b % 5) && Number.isFinite(b)).map((c) => Math.pow(2, c))
  );
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
let findMaleAndFemale = (data) =>
  data
    .reduce((a, b) => {
      if (b.gender === 'male' || b.gender === 'female') a.push(b.name);
      return a;
    }, [])
    .join(' and ');
...
let findShortest = (data) =>
  data.reduce((a, b) => (a.height < b.height ? b : a)).name;
