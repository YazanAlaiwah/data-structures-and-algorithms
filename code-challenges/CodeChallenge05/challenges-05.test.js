...
const listFoods = recipe => {
  ...
  recipe.ingredients.forEach(item => {
    ...
    for (let i = 0; i < item.length; i++) {
      if (item[i] === ' ' && spaceCount === 1) {
        result.push(item.slice(i + 1, item.length));
        break;
      }
      if (item[i] === ' ') {
        spaceCount++;
...
const splitFoods = recipe => {
  ...
  recipe.ingredients.forEach(item => {
    item = item.split(' ');
    item = item.slice(2, item.length).join(' ');
    result.push(item);
    // result.push(item.split(' ').splice())
    // let spaceCount = 0;
    // for (let i = 0; i < item.length; i++) {
    //   if (item[i] === ' ' && spaceCount === 1) {
    //     result.push(item.slice(i + 1, item.length));
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  return numberOfCharacters < 0 ? str : str.slice(0, numberOfCharacters * -1);
};
...
const totalSumCSV = str => str.split(',').reduce((a, b) => a + b * 1, 0);
...
------------------------------------------------------------------------------------------------ */
let obj = { a: true, e: true, i: true, o: true, u: true };
...
const extractVowels = str => {
  ...
  let extra = [];
  str = str.split('');
  str.slice().forEach((e, i) => {
    if (obj[e]) {
      extra.push(str.splice(i - count, 1));
      count++;
    }
  });
  return [str.join(''), extra.sort().join('')];
};
