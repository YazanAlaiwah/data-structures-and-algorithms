...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((element) => {
    if (element.name === character) {
      resulte = element.children.length > 0;
    }
  });
  return resulte;
};
...
const hasChildrenEntries = (arr, character) => {
  let resulte = false;
  arr.forEach((element) => {
    let arrObj = Object.entries(element);
    if (arrObj[0][1] === character) {
      resulte = arrObj[2][1].length > 0;
    }
  });
  return resulte;
};
...
const totalCharacters = (arr) =>
  arr.reduce((a, b) => (a += b.children.length + (b.spouse ? 1 : 0) + 1), 0);
...
const houseSize = (arr) =>
  arr.map((obj) => ({
    house: obj.house,
    members: obj.children.length + (obj.spouse ? 1 : 0) + 1,
  }));
...
const houseSurvivors = (arr) =>
  arr.map((obj) => ({
    ...
    members:
      obj.children.length +
      (obj.spouse ? (deceasedSpouses.includes(obj.spouse) ? 0 : 1) : 0) +
      1,
  }));
