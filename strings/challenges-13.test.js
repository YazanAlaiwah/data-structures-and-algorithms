...
const findHappiness = (arr) => arr.filter((a) => a.includes(':)'));
...
const standardizePhoneNumbers = (arr) =>
  arr.map(
    (a) => a.substring(1, 4) + a.substring(6, 9) + a.substring(10, a.length)
  );
...
const allHappy = (arr) => arr.filter((a) => !a.includes(':)')).length === 0;
...
const findAnything = (arr, target) => arr.filter((a) => a.includes(target));
...
const findEvery = (arr, target) =>
  arr.filter((a) => !a.includes(target)).length === 0;
...
const unenrollBrook = (arr) =>
  arr.map((a) => a.filter((b) => !b.includes('Brook')));
...
const sortByDay = (arr) =>
  daysOfWeek.map((a) => arr.filter((b) => b.includes(a)));
